...
export async function POST(req: Request) {
  ...
  const limited = await rateLimitRegister(clientKey);
  if (!limited.ok) {
  ...
  // Registration never auto-logs-in (anti-enumeration): no cookies are set.
  // The client routes the user to the sign-in screen.
  return attachRequestId(req, NextResponse.json({ ok: true, pending: true }));
}
