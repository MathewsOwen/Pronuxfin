...
type Props = {
  /** Com WebGL por baixo, orbs CSS ficam mais discretos para não competir com o 3D. */
  intensity?: "full" | "subtle";
};

/** Camadas CSS sobre o fundo — grade, orbs e scan line. */
export function AuthBrandCanvas({ intensity = "full" }: Props) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      {!subtle ? (
        ...
        <>
          <div className="absolute -left-16 top-[20%] size-[280px] rounded-full bg-primary/12 blur-[90px] motion-safe:animate-pulse-soft" />
          <div className="absolute bottom-[10%] right-0 size-[240px] rounded-full bg-cognitive/10 blur-[80px] motion-safe:animate-pulse-soft [animation-delay:1.4s]" />
        </>
      )}

      <div
        className={subtle ? "absolute inset-0 opacity-[0.22]" : "absolute inset-0 opacity-[0.35]"}
        style={{
          backgroundImage: `
